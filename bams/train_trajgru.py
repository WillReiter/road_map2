...
sys.path.append('/media/data1/bams')
...
def parse_args():
    ...
    parser.add_argument('--valid_and_save_checkpoint_iterations', help='valid and save checkpoint', required=True,
                        type=int)
...
def train(args):
    ...
    if os.path.exists(model_save_dir) and len(os.listdir(model_save_dir)) > 0:
        checkpoint = torch.load(os.path.join(model_save_dir, natsorted(os.listdir(model_save_dir))[-1]))
    ...
    mult_step_scheduler = lr_scheduler.MultiStepLR(optimizer,
                                                   milestones=[160000, 480000, 800000],
                                                   gamma=gamma)
    ...
    ### logging info ###
    logging.basicConfig(filename=os.path.join(save_dir, 'train_trajgru{}.log'.format(str(IN_LEN + OUT_LEN))),
                        level=logging.INFO)
    logging.info('train trajgru begin...')
    logging.info(start_iter)
    logging.info(train_loss)
    print(f"train loss will be saved in train_trajgru{IN_LEN + OUT_LEN}.log")

    for i in range(start_iter + 1, max_iterations + 1):
        ...
        if i == 1:
            ...
            load_file = os.path.join("/media/data8T_3/bams/experiments/",
                                     'encoder_forecaster_200000.pth')
        ...
        if i % valid_iteration_interval == 0:
            ...
            torch.save({
                ...
                'loss': train_loss,
            }, os.path.join(model_save_dir, 'encoder_forecaster_{}.pth'.format(i)))
